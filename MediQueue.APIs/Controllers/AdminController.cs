...
foreach (var clinic in clinics)
  {
     var user = users.FirstOrDefault(u => u.Id == clinic.UserId.ToString());
    var address = await _unitOfWork.Repository<Core.Entities.ClinicAddress>()
       .GetByIdAsync(clinic.Id);
    
            // Calculate average rating
        var ratings = await _unitOfWork.Repository<Core.Entities.ClinicRating>().GetAllAsync();
 var clinicRatings = ratings.Where(r => r.ClinicId == clinic.Id).ToList();
