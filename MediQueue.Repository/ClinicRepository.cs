...
public class ClinicRepository : GenericRepository<ClinicProfile>, IClinicRepository
{
    ...
    public async Task<ClinicProfile?> GetClinicByUserIdAsync(int userId)
    {
...
query = query.Where(c => c.Specialty.ToLower().Contains(specialty.ToLower()));
  ...
  if (!string.IsNullOrEmpty(city))
 {
    ...
    public async Task<bool> ClinicExistsForUserAsync(int userId)
    {
        return await _context.ClinicProfiles.AnyAsync(c => c.UserId == userId);
    }
}
